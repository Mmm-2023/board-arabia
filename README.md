...
- Email: Supabase Edge Functions call the **Gmail API** as `cindy@nammco.com` (dry-run audit when Workspace credentials are unset). No Resend.
...
| `GMAIL_FROM` | Optional | Defaults to `"Board Arabia" <cindy@nammco.com>` |
...
| Live Workspace delivery | Dry-run until Gmail secrets exist | Edge calls `gmail.googleapis.com` users.messages.send. From `cindy@nammco.com`. Dry-run while `GMAIL_REFRESH_TOKEN` (or the service-account JSON) is unset |
